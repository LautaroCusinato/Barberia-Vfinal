-- Extiende la auditoría técnica para registrar la reconciliación de la
-- suscripción sandbox existente. No habilita producción ni agrega acciones
...
begin
  ...
  if v_action not in ('config-status', 'sync-plans', 'checkout', 'external-status', 'reconcile-sandbox') then
  ...
  values (
    ...
    jsonb_build_object(
      ...
      'preapproval_id', case when v_metadata ? 'preapproval_id' then left(v_metadata->>'preapproval_id', 120) end,
      'idempotent', case when v_metadata->>'idempotent' in ('true', 'false') then (v_metadata->>'idempotent')::boolean else null end
    )
