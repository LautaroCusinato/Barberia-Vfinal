...
function mercadoPagoSecret(environment: MercadoPagoEnvironment, kind: 'token' | 'webhook') {
  ...
  // Environment-scoped secrets are mandatory. Never fall back to the other
  // environment: a sandbox binding must be unable to consume production
  // credentials even when a legacy variable happens to be present.
  return scoped
}
...
/**
 * Associated-plan subscription flow for an explicitly resolved environment.
 * The browser supplies only a one-time card_token_id generated by Mercado
 * Pago.js/Card Payment Brick; the token, plan, seller and amount remain
 * server-side. Production keeps its independent financial guards, while the
 * sandbox path is restricted to the fixed technical tenant and TEST seller.
 */
export async function mercadoPagoSubscription(input: { environment: MercadoPagoEnvironment; tenantId: number; externalPlanId: string; cardTokenId: string; payerEmail: string; externalReference: string; backUrl: string; idempotencyKey: string }) {
  if (input.environment === 'production') {
    ...
    if (input.tenantId !== 6) throw new ProviderError('Mercado Pago sandbox sólo permite el tenant técnico autorizado.', 403, 'sandbox_scope_required')
    await mercadoPagoSandboxIdentity()
  }
  ...
  const token = configuredMercadoPagoAccessToken({ allowProduction: input.environment === 'production', environment: input.environment })
  const base = input.environment === 'production'
    ? PRODUCTION_API_BASE_URL
    : (Deno.env.get('MERCADOPAGO_API_BASE_URL') || 'https://api.mercadopago.com').replace(/\/$/, '')
  ...
  const expectedSellerId = input.environment === 'production'
    ? Number(Deno.env.get('MERCADOPAGO_PRODUCTION_SELLER_ID'))
    : EXPECTED_MERCADO_PAGO_SANDBOX_SELLER_ID
  if (Number(currentUser?.id) !== expectedSellerId) throw new ProviderError(input.environment === 'production' ? 'La credencial no pertenece al vendedor productivo autorizado.' : 'La credencial no pertenece al vendedor sandbox autorizado.', 409, input.environment === 'production' ? 'production_seller_mismatch' : 'sandbox_seller_mismatch')
  const plan = await responseJson(await fetch(`${base}/preapproval_plan/${encodeURIComponent(input.externalPlanId)}`, { headers }), 'Mercado Pago')
  const expectedApplicationId = input.environment === 'production'
    ? Number(Deno.env.get('MERCADOPAGO_PRODUCTION_APPLICATION_ID'))
    : EXPECTED_MERCADO_PAGO_SANDBOX_APPLICATION_ID
  const expectedAmount = input.environment === 'production' ? 30000 : 15000
  if (Number(plan?.collector_id) !== expectedSellerId || Number(plan?.application_id) !== expectedApplicationId || Number(plan?.auto_recurring?.transaction_amount) !== expectedAmount || String(plan?.auto_recurring?.currency_id || '').toUpperCase() !== 'ARS' || Number(plan?.auto_recurring?.frequency) !== 1 || String(plan?.auto_recurring?.frequency_type || '').toLowerCase() !== 'months') {
    throw new ProviderError(input.environment === 'production' ? 'El plan productivo no coincide con el contrato Starter autorizado.' : 'El plan sandbox no coincide con el contrato Starter autorizado.', 409, input.environment === 'production' ? 'production_plan_mismatch' : 'sandbox_plan_mismatch')
  }
  const body = {
...
/** Backwards-compatible production-only entry point. */
export async function mercadoPagoProductionSubscription(input: { tenantId: number; externalPlanId: string; cardTokenId: string; payerEmail: string; externalReference: string; backUrl: string; idempotencyKey: string }) {
  return mercadoPagoSubscription({ ...input, environment: 'production' })
}
...
export async function mercadoPagoExternalStatus(input: { externalId: string; kind: 'checkout' | 'subscription'; environment: MercadoPagoEnvironment }) {
  const environment = mercadoPagoEnvironment(input.environment)
  const token = configuredMercadoPagoAccessToken({ allowProduction: environment === 'production', environment })
  if (environment === 'sandbox') await mercadoPagoSandboxIdentity()
