...
assert.match(node('Construir respuesta segura')?.parameters?.jsCode || '', /mode:'production-controlled'/)
assert.match(node('Construir respuesta segura')?.parameters?.jsCode || '', /resultReference:validation\.intent\+':controlled'/)
assert.doesNotMatch(node('Construir respuesta segura')?.parameters?.jsCode || '', /mode:'shadow'|:shadow/)
...
assert.match(provisioningFunction, /last_error_code: safeErrorCode\(error\)/)
assert.match(provisioningFunction, /last_error_message: null/)
...
assert.match(provisioningFunction, /last_error_code: row\.last_error_code \|\| null/)
...
assert.match(generator, /mode:'production-controlled'/)
assert.match(runbook, /does not persist[\s\S]*conversational booking state/i)
