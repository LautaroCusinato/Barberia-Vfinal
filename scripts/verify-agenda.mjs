...
const turnoRowSource = readFileSync(resolve('src', 'components', 'TurnoRow.jsx'), 'utf8')
const billingSource = readFileSync(resolve('src', 'pages', 'Billing.jsx'), 'utf8')
...
assert.match(appSource, /get_billing_portal/, 'WhatsApp debe verificar la habilitación comercial antes de activarse')
...
assert.match(sidebarSource, /Ver facturación y planes/, 'WhatsApp debe ofrecer una salida clara hacia Facturación cuando está bloqueado')
assert.doesNotMatch(turnoRowSource, /className=\{`agenda-status/, 'La tarjeta de agenda no debe duplicar el estado con un badge redundante')
assert.match(turnoRowSource, /agenda-item--notes-open/, 'La tarjeta activa debe elevarse cuando se abren notas')
assert.match(billingSource, /commercialMissing/, 'Billing debe distinguir ausencia comercial de error técnico')
assert.match(settingsSource, /invitationState/, 'Colaboradores debe mostrar estados de invitación')
