...
assert.equal(recommendPrice({ employees: 0 }).monthly, 50000)
assert.equal(recommendPrice({ employees: 4, whatsapp: true }).monthly, 50000)
assert.equal(recommendPrice({ employees: 12, customization: true }).monthly, 50000)
assert.equal(recommendPrice({ employees: 12, customization: true }).plan, 'starter')
assert.match(recommendPrice({ employees: 1 }).rationale[0], /Plan único: Austral/)
