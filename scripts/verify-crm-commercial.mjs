...
for (const contract of ['PAGE_SIZE', 'Importar CSV', 'Exportar', 'set_crm_lead_stage', 'calculate_crm_lead_score', 'do_not_contact', 'merge_crm_leads']) assert.match(page, new RegExp(contract), `falta interfaz ${contract}`)
for (const contract of ['parseLeadsCsv', 'isDangerousCsvValue', 'FIELD_ALIASES']) assert.match(csv, new RegExp(contract), `falta parser ${contract}`)
