...
const actionInbox = read('src/components/CRMActionInbox.jsx')
const outreachQueue = read('src/components/CRMOutreachQueue.jsx')
...
assert.match(agent, /if \(busy\) return/, 'el agente comercial debe bloquear acciones duplicadas')
assert.match(actionInbox, /if \(saving\) return/, 'las acciones CRM deben bloquear doble submit')
assert.match(actionInbox, /disabled=\{saving\}/, 'las acciones CRM deben exponer estado disabled')
assert.match(outreachQueue, /if \(!selected \|\| !canWrite \|\| saving\) return/, 'el registro de outreach debe bloquear doble submit')
assert.match(outreachQueue, /aria-busy=\{saving\}/, 'el registro de outreach debe anunciar el estado de guardado')
