...
for (const route of ['checkout', 'status', 'sync-plans']) assert.match(api, new RegExp(`route === '${route}'`), `falta ruta ${route}`)
for (const name of ['verifyMercadoPago', 'verifyPayPal', 'mercadoPagoResource', 'paypalResource']) assert.match(hooks + providers, new RegExp(name), `falta ${name}`)
