...
export default function App() {
  ...
  const saveTurno = async ({ paciente, telefono, clienteId, fecha, hora, motivo, estado, servicio_id, barbero_id, precio, duracion }, existingId) => {
    ...
    const dbPayload = { paciente, fecha, hora, motivo, estado, servicio_id, barbero_id, precio, duracion_min: duracion, cliente_id: finalClienteId }
  ...
  const addPaciente = async (datos) => {
    if (isSupabaseConfigured) {
      const { data, error } = await supabase.from('clientes').insert({ ...datos, barberia_id: BARBERIA_ID }).select()
      if (error) {
