...
const STATE_COPY = {
  ...
  STATUS_UNAVAILABLE: { label: 'Estado no disponible', tone: 'error', description: 'No pudimos verificar la conexión. Reintentá la consulta antes de preparar, reconectar o desconectar el canal.' },
}
...
export default function WhatsAppConnectionPanel({ barberiaId, demoMode = false }) {
  ...
  const invoke = useCallback(async (action) => {
    ...
    try {
      ...
      setError(safeMessage(invokeError))
    } finally {
      workingRef.current = false
      setWorking(false)
    }
  }, [barberiaId, demoMode, statusUnavailable])
