...
export default function OnboardingChecklist({ barberiaId, onNavigate, demoMode = false, whatsappStatus = {} }) {
  ...
  const labelForItem = (item) => {
    ...
    if (whatsappDisplay.entitlementLabel) return 'WhatsApp conectado · automatización requiere plan'
    return whatsappDisplay.automationLabel ? 'WhatsApp conectado · automatización pendiente' : 'WhatsApp conectado'
  }
  ...
  return (
    <div className="onboarding-checklist panel fade-in">
      ...
      {guideOpen && <div className="guide-panel"><div className="guide-heading"><MessageCircle size={17} /><strong>{whatsappDisplay.connectionState === 'connected' ? 'WhatsApp conectado' : whatsappDisplay.connectionState === 'unavailable' ? 'Estado no disponible' : 'Conectar WhatsApp'}</strong><button className="btn-icon-plain" onClick={() => setGuideOpen(false)} aria-label="Cerrar">×</button></div><p>{whatsappDisplay.connectionState === 'connected' ? (whatsappDisplay.whatsappReady ? 'La conexión y la automatización están habilitadas para este negocio.' : 'La conexión técnica está activa, pero la automatización todavía requiere habilitación operativa.') : whatsappDisplay.connectionState === 'unavailable' ? 'No pudimos verificar la conexión. Reintentá desde Configuración antes de preparar o reconectar el canal.' : 'La conexión se prepara de forma aislada para este negocio y se valida antes de habilitar mensajes.'}</p><ol>{whatsappDisplay.connectionState === 'connected' ? <li>{whatsappDisplay.whatsappReady ? 'El canal está listo para atender mensajes.' : 'Completá la habilitación operativa antes de anunciar respuestas automáticas.'}</li> : whatsappDisplay.connectionState === 'unavailable' ? <li>Actualizá el estado cuando la conexión vuelva a estar disponible.</li> : <><li>Entrá a Configuración y elegí “Preparar conexión”.</li><li>Revisá el estado y completá la vinculación desde el teléfono autorizado cuando esté disponible.</li><li>El canal permanece en modo de validación hasta que una persona autorizada lo habilite.</li></>}</ol><div className="guide-note"><CircleHelp size={15} /> Este asistente no envía mensajes ni modifica la configuración productiva.</div></div>}
    </div>
  )
}
