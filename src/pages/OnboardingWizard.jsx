    ...
    <div className="onboarding-shell">
      ...
      <main className="onboarding-layout">
        <aside className="onboarding-progress" aria-label="Progreso de configuración">
          <p className="auth-kicker">Tu prueba gratuita</p><h1 className="onboarding-heading">Empezá en minutos</h1><p className="auth-copy">Sin tarjeta. 15 días para probar toda la operación.</p>
        ...
        <section className="onboarding-card fade-in" key={step}>
          ...
          {step === 7 && <div className="review-card"><div className="review-row"><span>Negocio</span><strong>{form.nombre || '—'}</strong></div><div className="review-row"><span>Rubro</span><strong>{selectedVertical?.nombre || form.vertical}</strong></div><div className="review-row"><span>Ubicación</span><strong>{catalog.paises?.find((item) => item.codigo === form.pais)?.nombre || form.pais}</strong></div><div className="review-row"><span>Configuración</span><strong>{form.idioma} · {form.moneda}</strong></div><div className="trial-callout"><Sparkles size={17} /><span><strong>Se activa una prueba gratuita de 15 días</strong><small>Incluye configuración inicial, agenda y reservas. No se crea información ficticia.</small></span></div></div>}
