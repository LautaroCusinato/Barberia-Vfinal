...
export default function Landing({ vertical = 'custom' }) {
  ...
  return (
    <main className="marketing-sections">
      ...
      <section id="planes" className="marketing-section marketing-pricing-section"><div className="marketing-container"><div className="marketing-section-heading"><span className="marketing-kicker">Un solo plan</span><h2>Austral para ordenar tu operación.</h2><p>ARS 50.000 por mes, con 15 días gratis para probar el flujo completo. Mercado Pago está pausado: podés crear tu cuenta o consultar con el equipo.</p></div><div className="marketing-plan-grid">{plans.map((plan) => { const salesHref = getSalesWhatsAppHref(plan); return <article className="marketing-plan-card" key={plan.codigo}><div className="marketing-plan-card-heading"><div><span className="marketing-plan-code">Austral</span><h3>{plan.nombre}</h3></div><span className="marketing-plan-trial">{plan.trial_dias || COMMERCIAL_TRIAL_DAYS} días de prueba</span></div><p className="marketing-plan-description">{plan.descripcion}</p><strong className="marketing-plan-price">{formatPlanPrice(plan, locale)}<small> / mes</small></strong><ul>{planFeatures(plan).map((feature) => <li key={feature}><Check size={15} /> {feature}</li>)}</ul><div className="marketing-plan-actions"><a className="marketing-button secondary" href="/registro">Probar gratis 15 días <ArrowRight size={15} /></a>{salesHref && <a className="marketing-text-link" href={salesHref} target="_blank" rel="noreferrer">Consultar por WhatsApp <MessageCircle size={15} /></a>}</div></article> })}</div><p className="marketing-pricing-note marketing-pricing-note--branches">¿Tenés más de una sucursal? Lo vemos según tu caso.</p><p className="marketing-pricing-note"><ShieldCheck size={15} /> El trial se inicia al completar el onboarding. La continuidad se coordina manualmente por WhatsApp.</p></div></section>
