...
La validación estática y los contratos existentes pasan. La matriz QA se ejecutó sin pagos ni checkouts externos: **192/192 autenticados** y **72/72 públicos**. El mock de billing verificó estados, checkout, usuario sin tenant bloqueado y reconciliación idempotente. Los fixtures Auth faltantes de la auditoría previa se restauraron mediante el seeder idempotente. No se ejecutaron pagos, checkouts, Mercado Pago productivo ni mutaciones sobre Barbería Central o Barbería Nueva. El catálogo y el estado `subscription_missing` siguen siendo estados comerciales, no errores técnicos.
